module.exports = class Game{
    ...
    addRandomScores() {
        var scores = new Array(this.frames);
        for (var i = 0; i < this.frames; i++) {
            scores[i] = new Array(3);
        }

        for (var i = 0; i < this.frames; i++){
            var firstShot = Math.floor(Math.random() * 10);
            scores[i][0] = firstShot;
            scores[i][1] = Math.floor(Math.random() * (Math.abs(firstShot - 10))) ;
            scores[i][2] = 0;
            scores[i][3] = 0;
            if (i == this.frames - 1 && (this.IsStrike(scores[i] || this.IsSpare(scores[i])))) {
    ...
    rules(frame, i) {
        var score = this.player.finalScore;
